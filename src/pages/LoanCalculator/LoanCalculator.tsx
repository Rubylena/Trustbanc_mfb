  ...
  return (
    <div>
      <div className="calculator-card">
        ...
        <Link to='/quick-loans'><p className="tw-underline tw-text-blue tw-p-2 tw-pl-5 tw-font-medium"> &lArr; Quick loans</p></Link>
        ...
        <div className="calculator-body2">
          ...
          <div className="amount-card tw-flex-wrap md:tw-flex-nowrap">
            <div className="amount p-2 md:p-4">
              <h3 className="tw-text-3xl md:tw-text-4xl">₦ {Add.toLocaleString()}</h3>
              <p>Total payment</p>
            </div>
            <div className="amount p-2 md:p-4">
              <h3 className="tw-text-3xl md:tw-text-4xl">₦ {monthlySum.toLocaleString()}</h3>
              <p>Monthly payment</p>
            </div>
          </div>
          <form className="cal-form" onSubmit={calcSum}>
            <div className="container px-5">
              <div className="form-floating mb-3 mt-3">
                ...
                {error && num1 == 0 && <p className="tw-text-red-700 tw-text-left tw-font-medium tw-text-sm tw-pl-2">{error}</p>}
              </div>
              <div className="form-floating" style={{ width: '100%' }}>
                <input
                  ...
                  disabled
                />
                <label htmlFor="floatingPassword" style={{ width: '100%' }}>Interest Rate by 3%</label>
              </div>
              <div className="form-floating mt-3" style={{ width: '100%' }}>
                <input
                  ...
                  min="0"
                  value={num3 || ""}
                  onChange={(e) => { setNum3(Number(e.target.value)); setError(""); }}
                  className="form-control"
                  id="floatingPassword"
                  placeholder="Tenor by month"
                />
                <label htmlFor="floatingPassword" style={{ width: '100%' }}>Loan Tenor (by month)</label>
                {error && num3 == 0 && <p className="tw-text-red-700 tw-text-left tw-font-medium tw-text-sm tw-pl-2">{error}</p>}
              </div>
